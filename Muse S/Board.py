...
class Board(BoardShim):
    def __init__(
        ...
        self.board_id = board_id if board_id is not None else get_board_id(
            data_type, hardware, model)
        ...
        if self.board_id in use_serial_port:
            self.params.serial_port = (
                serial_port if serial_port is not None else get_serial_port(
                    self.board_id,)  # portstring = '/dev/ttyUSB')
            )
